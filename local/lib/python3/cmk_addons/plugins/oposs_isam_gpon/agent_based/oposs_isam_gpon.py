...
# GPON line rates per ITU-T G.984 (bits per second).
# OLT Tx = downstream toward ONTs, OLT Rx = upstream from ONTs.
GPON_DOWNSTREAM_BPS = 2_488_320_000
GPON_UPSTREAM_BPS = 1_244_160_000

# Byte counters fetched from gponOltSidePonUtilPmCurrentIntervalTable.
# These are PM counters that reset at the start of every 15-minute window,
# so the check turns them into bytes/sec rates via _interval_rate().
_BYTE_COUNTER_FIELDS = [
    "TxUcastBytes", "TxMcastBytes", "TxBcastBytes",
    "TxTotalBytes", "RxTotalBytes",
    "TxUcastDropBytes", "RxTotalDropBytes",
    "TxMcastDropBytes", "TxBcastDropBytes", "TxTotalDropBytes",
]

# Utilization metrics derived from the rates above.
_UTIL_DERIVATIONS = [
    ("TxUcastUtil", "TxUcastBytes", GPON_DOWNSTREAM_BPS),
    ("TxMcastUtil", "TxMcastBytes", GPON_DOWNSTREAM_BPS),
    ("TxBcastUtil", "TxBcastBytes", GPON_DOWNSTREAM_BPS),
    ("TxTotalUtil", "TxTotalBytes", GPON_DOWNSTREAM_BPS),
    ("RxTotalUtil", "RxTotalBytes", GPON_UPSTREAM_BPS),
]
...
def _parse_gpon(string_table):
    """Parse GPON data from 5 SNMP trees."""
    result = {}
    ...
    # Block 2: PM - byte counters (8-byte big-endian, reset every 15-min PM
    # window — rate computation happens in the check function).
    _parse_bitmap_block(
    ...
    # Block 3: Description
    _parse_bitmap_block(result, string_table[3], ["description"], "")

    # Block 4: Standard IF admin/oper status
    _parse_bitmap_block(result, string_table[4], ["adminstatus", "operstatus"], "")
...
def _check_gpon(item, section) -> CheckResult:
    ...
    now = time.time()
    rates = {}
    for metric_key in _BYTE_COUNTER_FIELDS:
        if metric_key not in dic:
            continue
        rate = _interval_rate(value_store, metric_key, now, dic[metric_key])
        if rate is not None:
            rates[metric_key] = rate
            yield Metric("oposs_isam_%sRate" % metric_key, rate)

    # Utilization derived from byte rates against the GPON line rate.
    # Replaces the device-reported Util fields, which were also tied to
    # the resetting PM interval and so equally distorted.
    for util_key, bytes_key, line_rate_bps in _UTIL_DERIVATIONS:
        if bytes_key in rates:
            util_pct = rates[bytes_key] * 8.0 / line_rate_bps * 100.0
            yield Metric("oposs_isam_%s" % util_key, util_pct)

    # Plain gauges from the same SNMP block.
    for metric_key in ["NumActiveOnts", "RxDBACongestionTime"]:
...
snmp_section_oposs_isam_gpon = SNMPSection(
    ...
    fetch=[
        ...
        # 2: PM - byte counters (8-byte OIDBytes, reset every 15 min;
        # check turns them into rates and derives utilization)
        SNMPTree(
            base=".1.3.6.1.4.1.637.61.1.35.21.57.1",
            oids=[
                OIDEnd(),
        ...
        # 4: Standard IF admin/oper status
        SNMPTree(
